...
def _assert_open_purges_stale_skill_doc_seed(tmp_path) -> None:
...
def _assert_open_merges_duplicate_skill_doc(tmp_path) -> None:
    pack = materialize_skill_doc_guard_pack(tmp_path / "guard")
    seed = load_polluted_seed()
    seed_polluted_skill_doc_db(
        db_path=pack.user_tier_db_path,
...
def _assert_open_purges_executor_execute_artifact(tmp_path) -> None:
    pack = materialize_skill_doc_guard_pack(tmp_path / "guard")
    seed = load_polluted_seed()
    seed_polluted_skill_doc_db(
        db_path=pack.user_tier_db_path,
