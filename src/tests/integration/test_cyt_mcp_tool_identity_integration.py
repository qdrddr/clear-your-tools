...
def _backend_tools_as_fastmcp() -> list[Tool]:
    ...
    for raw in load_backend_tools():
        ...
        _handler.__name__ = name  # type: ignore[attr-defined]
...
async def test_refresh_catalog_cache_populates_explicit_identity_fields(tmp_path: Path) -> None:
    ...
    server._list_tools = _list_tools  # type: ignore[method-assign]
    ...
    exported_graphify = next(item for item in exported if item.get("name") == "graphify_query_graph")
