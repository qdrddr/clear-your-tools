...
the keyring (or env). Skipped by default; opt in with::
    ...
    uv run pytest src/tests/integration/test_executor_connection_health_integration.py --run-integration -s
...
from tests.support.credential_helpers import CI_CREDENTIAL_STUBS
