...
Run one scenario from the CLI:
    ...
    uv run python src/tests/integration/test_llm_prune_integration.py --mode tools --agent cursor --rule .debug/rules/cyt-injection.mdc \
...
``~/.config/cyt/cache/executor-catalog/*.json`` and skills entries):
    ...
    uv run python src/tests/integration/test_llm_prune_integration.py --mode real --agent cursor \
...
def _format_pruned_tools(catalog: dict[str, Any], selected_ids: set[int]) -> str:
    ...
    pruned = apply_selector_ids_to_catalog(
        ...
        metadata,
        dict.fromkeys(selected_ids, 100),
        list_keys,
    )
    inject_tools = [
