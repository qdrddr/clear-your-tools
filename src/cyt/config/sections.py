...
def inject_via_map_from_config(cfg: dict[str, Any]) -> dict[str, str]:
    """Build per-agent inject_via map from merged config."""
    defaults = inject_via_defaults_from_bundled(cfg)
...
def hallucination_gate_enabled_raw(cfg: dict[str, Any]) -> object | None:
    """Hallucination gate flag from ``defaults.hallucination_gate`` or legacy top-level key."""
    defaults = cfg.get("defaults")
    if isinstance(defaults, dict):
        gate = defaults.get("hallucination_gate")
        if isinstance(gate, dict) and "enabled" in gate:
            return gate.get("enabled")
    gate = cfg.get("hallucination_gate")
    if isinstance(gate, dict) and "enabled" in gate:
        return gate.get("enabled")
    return None


def cursor_rule_file_enabled_raw(cfg: dict[str, Any]) -> object | None:
...
def max_batch_workers_raw(cfg: dict[str, Any]) -> object | None:
    """Parallel prune workers from ``tools.max_batch_workers`` or legacy ``pruning`` key."""
