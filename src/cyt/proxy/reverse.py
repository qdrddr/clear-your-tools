...
def build_routes(proxy_cfg: dict[str, Any]) -> dict[str, tuple[str, str | None]]:
    ...
    for endpoint in reverse_cfg.get("endpoints", []):
        ...
        kind = entry.get("kind")
        base_url = entry.get("base_url") or entry.get("host_url")
        if not base_url:
            raise ValueError(f"No base_url configured for upstream: {endpoint}")
        routes[f"/{endpoint}"] = (str(base_url).rstrip("/"), kind)
    if not routes:
