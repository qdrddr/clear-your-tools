...
def _excluded_by_frontmatter_gate(
    query: str,
...
def _reconstruct_matched_skills(
    by_doc: dict[str, list[dict[str, Any]]],
    frontmatter_by_doc: dict[tuple[str, str], str | None],
    *,
    max_tokens: int,
) -> list[MatchedSkill]:
...
def search_skills(
    ...
    *,
    ...
    excluded = _excluded_by_frontmatter_gate(query, entries, config=config)
    eligible = [entry for entry in entries if (entry.entry_dir, entry.doc_id) not in excluded]
    if not eligible:
        return []

    corpus = _build_corpus(eligible)
    md_items = corpus.get("md")
    if not isinstance(md_items, list) or not md_items:
        return []
    ...
    return _reconstruct_matched_skills(
        _group_survivors_by_doc(survivors),
        _frontmatter_by_doc(eligible),
