...
use cyt_indexer::cache::lru::LruCache;
use cyt_indexer::cache::test_guard::CacheConfigTestGuard;
use cyt_indexer::cache::{
    CachePolicy, ensure_skills_registry, ensure_skills_registry_from_specs, hot_cache_len,
    parse_skill_source_specs_json, read_chunk_body, store_merged_document,
};
...
fn inline_content_skips_disk_source_read() {
    ...
    let specs = parse_skill_source_specs_json(source.as_array().unwrap()).unwrap();
