...
fn entry_layout_paths() {
    ...
    assert_eq!(entry, PathBuf::from("/catalog/abc123"));
    assert_eq!(
        page_index_path(&entry),
        PathBuf::from("/catalog/abc123/nodes/page_index.json")
    );
    assert_eq!(
        node_md_path(&entry, 0),
        PathBuf::from("/catalog/abc123/nodes/n0.md")
    );
    let variant = chunk_variant_dir(&entry, "BM25", "hash1");
    assert_eq!(variant, PathBuf::from("/catalog/abc123/chunks/bm25/hash1"));
    assert_eq!(
        chunk_md_path(&variant, 3),
        PathBuf::from("/catalog/abc123/chunks/bm25/hash1/c3.md")
    );
