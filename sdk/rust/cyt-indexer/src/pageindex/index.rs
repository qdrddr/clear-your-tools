...
pub fn md_to_tree(markdown_content: &str, source_path: &str, config: &PageIndexConfig) -> MdIndexResult {
    ...
    let mut tree_structure = build_tree_from_nodes(&nodes_with_content);

    if config.if_add_node_id {
        write_node_id(&mut tree_structure, 0);
    }

    let structure = format_structure_for_output(&tree_structure, config);
